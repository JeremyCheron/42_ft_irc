...
void CommandHandler::handleJoin(const std::vector<std::string> &params, Client &client, Server &server)
{
	...
	if (params.size() < 2)
		return MessageHelper::sendMsgToClient(&client, MessageHelper::errNeedMoreParams("JOIN"));
	if (client.isRegistered())
	{
...
void CommandHandler::handleWhois(const std::vector<std::string> &params, Client &client, Server &server)
{
	...
	std::string rpl311 = ":ft_irc 311 " + client.getNickname() + " " + target->getNickname() + " " + target->getUsername() + " localhost * :" + target->getUsername() + "\r\n";
	send(client.getFd(), rpl311.c_str(), rpl311.size(), 0);

	std::string rpl318 = ":ft_irc 318 " + client.getNickname() + " " + target->getNickname() + " :End of WHOIS list\r\n";
...
{
	if (params.size() < 3)
		return MessageHelper::sendMsgToClient(&client, MessageHelper::errNeedMoreParams("INVITE"));
	std::string targetNick = params[1];
	...
	if (!channel->getClients().count(&client))
		return MessageHelper::sendMsgToClient(&client, MessageHelper::errNotOnChannel(channelName));
	if (channel->hasMode('i') && !channel->isClientOperator(&client))
		return MessageHelper::sendMsgToClient(&client, MessageHelper::errNotChannelOperator(client.getNickname(), channelName));

	Client *targetClient = server.findClientByNickname(targetNick);
	if (!targetClient)
		return MessageHelper::sendMsgToClient(&client, MessageHelper::errNoSuchNick(targetNick));
	if (channel->getClients().count(targetClient))
		return MessageHelper::sendMsgToClient(&client, MessageHelper::errUserOnChannel(targetNick, channelName));
...
{
	if (params.size() < 2)
		return MessageHelper::sendMsgToClient(&client, MessageHelper::errNeedMoreParams("CAP"));
	(void)server;
